...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const userLogin = await axios.post('/api/login', {
        email,
        password,
      });

      if (userLogin.data.message) {
        setMessage(userLogin.data.message);
      } else {
        setValues({
          email: '',
          password: '',
        ...
        // if (userLogin.data.redirectTo) {
        //   window.location.href = userLogin.data.redirectTo;
